...
@parallel function diffusion1D_step!(T2, T, Ci, lam, dt, _dx)
    @inn(T2) = @inn(T) + dt*(lam*@inn(Ci)*(@d2(T)*_dx^2));
    return
end
...
function transfer_heat!(kmc::KineticMonteCarlo, T2, bc!::Function)
    @parallel diffusion1D_step!(T2, kmc.T, kmc.Ci, kmc.lam, kmc.dt, 1/kmc.dx)
    bc!(T2)
    kmc.T, T2 = T2, kmc.T
    Δt = -log(rand()) * kmc.dt
    kmc.t += Δt
