...
function KineticMonteCarlo(ℓx::Real, dx::Real, ℓy::Real, dy::Real,
                           ...
                           A::Real, EA::Real, M::Real, Tc::Real, Tg::Real, B::Real,
                           ΔT::Real, Cbed::Real, C0::Real, Cair::Real,
                           kbed::Real, k0::Real, kair::Real, k0mult::Real,
                           Tbed::Real, T0::Real, Tair::Real, v0::Real,
                           J::Real, Jm::Real, σ_init::Float64, maxrow::Int,
    ...
    KineticMonteCarlo(0.0, dt, 0.0, max_dt, max_Δt, χ, nhat, active, 
                      A, EA, M, Tc, Tg, B, ΔT, T, Cρ, k, k0mult,
                      dx, ℓx, dy, ℓy, igap, (jbed+1):jtop, igap, maxrow, 
                      1, true, T0, v0, 
                      jbed, C0, k0, J, Jm, solver, has_amorphint, 
                      has_meltint, d
                     )
end
...
function kmc_events(kmc::KineticMonteCarlo, bc!::Function, bcdT!::Function)
    ...
    for j=1:nj
        Threads.@threads for i=1:ni
            ...
            idx = (j-1)*ni + i
            if kmc.χ[i, j] < 1 && kmc.T[i, j] < kmc.Tc
                dχ = rand()*(1-kmc.χ[i, j])
                nbrχ = status_crystal_nbrs(kmc, i, j, ni, nj)
                dEA = -kmc.J*nbrχ
                rates[idx] = kmc.A/dχ*exp(-(kmc.EA + dEA)/(kmc.Tc - kmc.T[i, j]))
